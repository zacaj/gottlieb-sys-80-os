...
int main(int argc, char* argv[]) {
	if(argc < 3) {
		fprintf(stderr, "usage: split rom_folder type\n eg pinmame32_23/roms/game_name/ (note trailing slash)\ntype: sys3, sys4, sys6, sys7, or sys80");
		return 1;
	}
	char* folder = argv[1];
	FILE *in  = fopen("rom.bin", "rb");
	if(!in) {
		fprintf(stderr, "could not open rom.bin, errno %i", (errno));
		return 3;
	}
	char path[100];
	FILE *a, *b, *c, *aa, *bb, *cc;
	if (!strcmp(argv[2], "sys80")) {
		sprintf(path, "%su2_80.bin", folder);
		a = fopen(path, "wb");
		sprintf(path, "%su3_80.bin", folder);
		b = fopen(path, "wb");
		aa = fopen("u2_80.bin", "wb");
		bb = fopen("u3_80.bin", "wb");
